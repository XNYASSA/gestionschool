...
const COLONNES = [
  ...
  { titre: 'Montant perçu', cle: 'montant' }
]

const EXEMPLE = ['MAT001', 'Nkomo', 'Jean', '6ème A', '65000']

export default function ImporterVerificationsEconomat({ onImportTermine }) {
  ...
  const telechargerModele = async () => {
    ...
    const feuille = workbook.addWorksheet('Vérifications')
    ...
    a.href = url
    a.download = 'modele-import-verifications.xlsx'
    a.click()
  ...
  const handleFichier = async (e) => {
    ...
    try {
      ...
      if (lignesLues.length === 0) {
        setErreurLecture("Aucune ligne trouvée dans ce fichier.")
      }
  ...
  const lancerImport = async () => {
    ...
    setResultat(null)
    try {
      const res = await apiClient.importerVerifications(lignes)
      setResultat(res)
      onImportTermine?.()
    } catch (err) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 space-y-4">
      <p className="text-sm text-slate-500">
        Format : Matricule (ou Nom + Prénom + Classe), puis le montant global perçu pour cet élève.
      </p>
      ...
      {fichier && lignes.length > 0 && !resultat && (
        <div className="space-y-3">
          <p className="text-sm text-slate-600">{lignes.length} vérification(s) détectée(s) dans le fichier « {fichier.name} ».</p>
          <button
            onClick={lancerImport}
            disabled={importing}
            className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition disabled:opacity-50 flex items-center gap-2"
          >
            {importing ? <><Loader className="w-4 h-4 animate-spin" /> Import en cours...</> : <><Upload className="w-4 h-4" /> Importer {lignes.length} vérification(s)</>}
          </button>
        </div>
      )}
